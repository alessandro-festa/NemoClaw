...
| `NEMOCLAW_AIF_SERVER_URL` | *unset* | Default AIF operator base URL for `nemoclaw resync` (EPIC #114 US-156). Overridable per-invocation via `--server-url`. |
| `NEMOCLAW_AIF_API_KEY` | *unset* | Default AIF operator apikey for `nemoclaw resync` (EPIC #114 US-156). Overridable per-invocation via `--api-key`. |
